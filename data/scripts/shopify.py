...
def get_page(url, page, collection_handle=None):
    ...
    while True:
        try:
            TOTAL_REQUESTS += 1
            print('Getting new page... {}, TOTAL: {}'.format(page, TOTAL_REQUESTS))
    ...
    while products:
        for product in products:
            ...
            for i, variant in enumerate(product['variants']):
                ...
                row = {'sku': variant['id'], 'product_type': product_type,
                ...
                for k in row:
                    # this line is awful, yes, I made it bad, no, I don't care :)
                    row[k] = str(row[k].strip()) if (row[k] and type(row[k]) is str) else str(row[k]) if (row[k]) else ''
                yield row
        ...
        for col in get_page_collections(url):
            ...
            for product in extract_products_collection(url, handle):
                ...
                writer.writerow([str(title),
                                ...
                                product['price'],
                                product['stock'], product['product_url'], product['sku']])
                PRODUCTS_WRITTEN += 1
                print('TOTAL WRITTEN: {}'.format(PRODUCTS_WRITTEN))
...
if __name__ == '__main__':
    ...
    if len(args) > 0:
        ...
        if options.list_collections:
            ...
            extract_products(url, 'products-cl.csv', collections)